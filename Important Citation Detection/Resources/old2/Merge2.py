...
def citationdataset():
    fileName = 'annotatedUpdated.csv'
    SS = set()
    citationsCollection = set()
    with open(fileName) as f:
        for line in f:
            paper_id = line.split(',')[3].strip()
            SS.add(paper_id)
    directory = "C:\\Users\\kadnan\\Desktop\\Dataset\\Semantic Scholar\\Papers" 
#     directory = "C:\\Users\\kadnan\\Desktop\\Semantic Scholar\\Dataset\\Papers" 
    for paper in SS:
        if (os.path.isfile(os.path.join(directory, paper + '.json'))): 
            x_file = open(os.path.join(directory, paper + '.json'), "r")
            data = json.load(x_file)
            citations = data['citations']
            for citation in citations:
                citationId = str(citation['paperId'])
                citationsCollection.add(citationId)
        else:
            continue
            print paper
    print len(SS), len(citationsCollection)
    count = 0
#     directory = "C:\\Users\\kadnan\\Desktop\\Dataset\\Semantic Scholar\\Papers"
    directory = "C:\\Users\\kadnan\\Desktop\\Semantic Scholar\\Dataset\\Papers"
    for paper in citationsCollection:
        if (os.path.isfile(os.path.join(directory, paper + '.json'))): 
            continue
        else:
            count = count + 1
