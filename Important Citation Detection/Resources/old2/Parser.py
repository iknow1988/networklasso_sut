...
def sortACL():
    ACL = pickle.load(open("ACLneeded.p", "rb"))
    otputFile = open('sortedACLneeded.txt', 'w')
...
def readSemanticScholarPickle():
    SSdataset = dict()
    fileName = "papersSemanticScholar.txt"
    count = 0 
    with open(fileName) as f:
        for line in f:
            print count
...
def findSimilairty():
    ...
    with open(ACLfileName) as f1:
        for line1 in f1:
            data1 = line1.split('\t')
            if (len(data1)==2):
                paper_id_acl = data1[0].strip().replace('"', '')
