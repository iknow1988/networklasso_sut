...
erel = math.pow(10, -3)
maxIteration = 50
lambdaThreshold = 1001
lamb = 100
lambdaUpdateStepSize = 10
nodes = 20
partitions = 20
...
def shuffleDataset(trainingSet):
    dataset = np.zeros((nodes * trainSetSize, sizeOptimizationVariable + 1))
    ...
    for j in range(nodes):
        for i in range(trainSetSize):
            tempx = x_train[i * sizeOptimizationVariable:(i + 1) * sizeOptimizationVariable, j]
            tempy = y_train[i, j]
            dataset[count, 0:sizeOptimizationVariable] = tempx
            dataset[count, sizeOptimizationVariable:sizeOptimizationVariable + 1] = tempy 
            count = count + 1
    check = np.copy(dataset)
    np.random.shuffle(dataset)
    newset = np.zeros((trainSetSize * (sizeOptimizationVariable + 1), nodes))
    count = 0
    for j in range(nodes):
        for i in range(trainSetSize):
#             print count, i * sizeOptimizationVariable,(i + 1) * sizeOptimizationVariable,trainSetSize * sizeOptimizationVariable + i
            newset[i * sizeOptimizationVariable:(i + 1) * sizeOptimizationVariable, j] = dataset[count, 0:51]
            newset[trainSetSize * sizeOptimizationVariable + i, j] = dataset[count, sizeOptimizationVariable:sizeOptimizationVariable + 1]
            count = count + 1
...
def main():
    ...
    trainingSet = shuffleDataset(trainingSet)
    x_train = trainingSet[0:trainSetSize * sizeOptimizationVariable, :]
    y_train = trainingSet[trainSetSize * sizeOptimizationVariable: trainSetSize * (sizeOptimizationVariable + 1), :]
    ...
    while(lamb <= lambdaThreshold or lamb == 0):
        ...
        #print x
        print "True Variance"
        print np.var(a_true-x)
        #print "True Variance"
#         for i in range (sizeOptimizationVariable):
#             print np.var(a_true[i,:]-x[i,:])
